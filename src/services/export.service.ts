...
export class ExportService {
     ...
     */
    async exportBalanceSheetToExcel(data: any): Promise<ExcelJS.Buffer> {
        ...
        data.aset.accounts.forEach((account: any) => {
        ...
        data.liabilitas.accounts.forEach((account: any) => {
            worksheet.addRow([`  ${account.namaAkun}`, account.saldo]);
        });
        ...
        data.ekuitas.accounts.forEach((account: any) => {
            worksheet.addRow([`  ${account.namaAkun}`, account.saldo]);
        });
     ...
     */
    async exportIncomeStatementToExcel(data: any): Promise<ExcelJS.Buffer> {
        const workbook = new ExcelJS.Workbook();
        ...
        data.pendapatan.accounts.forEach((account: any) => {
        ...
        data.beban.accounts.forEach((account: any) => {
            worksheet.addRow([`  ${account.namaAkun}`, account.jumlah]);
        });
     ...
     */
    async exportBalanceSheetToPDF(data: any): Promise<Buffer> {
        return new Promise((resolve, reject) => {
            ...
            doc.fontSize(11)
                .font('Helvetica-Bold')
                .text(
                    `ASET = LIABILITAS + EKUITAS: ${data.balanced ? '✓ BALANCE' : '✗ NOT BALANCE'}`,
                    { align: 'center' }
                );
     ...
     */
    async exportIncomeStatementToPDF(data: any): Promise<Buffer> {
        return new Promise((resolve, reject) => {
            const doc = new PDFDocument({ margin: 50 });
            const chunks: Buffer[] = [];
            ...
            doc.fontSize(10)
                .font('Helvetica')
                .text(
                    `Periode: ${new Date(data.startDate).toLocaleDateString('id-ID')} - ${new Date(data.endDate).toLocaleDateString('id-ID')}`,
                    { align: 'center' }
                );
     ...
     */
    async exportCashFlowToExcel(data: any): Promise<ExcelJS.Buffer> {
        const workbook = new ExcelJS.Workbook();
     ...
     */
    async exportCashFlowToPDF(data: any): Promise<Buffer> {
        return new Promise((resolve, reject) => {
            const doc = new PDFDocument({ margin: 50 });
            const chunks: Buffer[] = [];
     ...
     */
    async exportEquityChangesToExcel(data: any): Promise<ExcelJS.Buffer> {
        const workbook = new ExcelJS.Workbook();
     ...
     */
    async exportEquityChangesToPDF(data: any): Promise<Buffer> {
        return new Promise((resolve, reject) => {
            const doc = new PDFDocument({ margin: 50 });
            const chunks: Buffer[] = [];
