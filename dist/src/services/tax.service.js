...
class TaxService {
    ...
    async calculatePPh21(data) {
        try {
            ...
            logger_1.default.error('Calculate PPh 21 error:', error);
            throw error;
        }
    }
    /**
    ...
    async calculatePPN(data) {
        try {
            ...
            logger_1.default.error('Calculate PPN error:', error);
            throw error;
        }
    }
    /**
     ...
     */
    async createTaxTransaction(data, requestingUserId) {
        try {
            ...
            const canCreate = ['ADMIN', 'TAX_OFFICER', 'ACCOUNTANT', 'SENIOR_ACCOUNTANT'].includes(requestingUser.role);
            if (!isSuperAdmin && !(isOwnCompany && canCreate)) {
                throw new auth_service_1.AuthenticationError('Anda tidak memiliki akses untuk membuat transaksi pajak');
            }
            ...
            logger_1.default.info(`Tax transaction would be created: ${data.tipePajak} - ${data.jumlahPajak}`);
            return {
            ...
            logger_1.default.error('Create tax transaction error:', error);
            throw error;
        }
    }
    /**
     * Get tax report
     */
    async getTaxReport(filters, requestingUserId) {
        try {
            ...
            logger_1.default.error('Get tax report error:', error);
            throw error;
        }
    }
}
