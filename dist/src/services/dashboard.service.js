...
class DashboardService {
    ...
    async getFinancialKPIs(filters) {
        ...
        // Get total revenue
        const revenue = await database_1.default.jurnalDetail.aggregate({
            where: {
                jurnal: {
        ...
        // Get total expenses
        const expenses = await database_1.default.jurnalDetail.aggregate({
            where: {
                jurnal: {
                    perusahaanId: filters.perusahaanId,
                    tanggal: { gte: startDate, lte: endDate },
        ...
        // Get total assets
        const assets = await database_1.default.chartOfAccounts.aggregate({
            where: {
                perusahaanId: filters.perusahaanId,
                tipe: 'ASET',
            },
        ...
        const liabilities = await database_1.default.chartOfAccounts.aggregate({
            where: {
                perusahaanId: filters.perusahaanId,
                tipe: 'LIABILITAS',
            },
    ...
    async getCashFlowSummary(filters) {
        ...
        const inflows = await database_1.default.jurnalDetail.aggregate({
            where: {
                jurnal: {
                    perusahaanId: filters.perusahaanId,
                    tanggal: { gte: startDate, lte: endDate },
        ...
        const outflows = await database_1.default.jurnalDetail.aggregate({
            where: {
                jurnal: {
                    perusahaanId: filters.perusahaanId,
                    tanggal: { gte: startDate, lte: endDate },
    ...
    async getRevenueAnalytics(filters) {
        ...
        const revenue = await database_1.default.$queryRaw `
            SELECT 
    ...
    async getExpenseAnalytics(filters) {
        const startDate = new Date(filters.startDate);
        const endDate = new Date(filters.endDate);
        const expenses = await database_1.default.$queryRaw `
            SELECT 
                DATE_TRUNC(${filters.groupBy || 'month'}, j."tanggal") as period,
    ...
    async getBalanceTrend(filters) {
        const startDate = new Date(filters.startDate);
        const endDate = new Date(filters.endDate);
        const trend = await database_1.default.$queryRaw `
            SELECT 
                DATE_TRUNC(${filters.groupBy || 'month'}, j."tanggal") as period,
    ...
    async getTopAccounts(filters) {
        ...
        const topAccounts = await database_1.default.jurnalDetail.groupBy({
            by: ['akunId'],
            where: whereClause,
            _sum: {
                debit: true,
